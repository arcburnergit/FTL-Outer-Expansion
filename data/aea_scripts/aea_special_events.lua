...
local specialEvents = {}
local targetFile = "data/events_aea_lua_events.xml"
local eventTagName = "aea_special_event"
local choiceTagName = "aea_choice"

script.on_load(function()
	local doc = RapidXML.xml_document(targetFile)
	for node in node_child_iter(doc:first_node("FTL") or doc) do
		if node:name() == eventTagName then
...
script.on_internal_event(Defines.InternalEvents.PRE_CREATE_CHOICEBOX, function(event)
	...
	if specialEvents[event.eventName] then
...
script.on_render_event(Defines.RenderEvents.CHOICE_BOX, function() end, function()
	...
	if commandGui.event_pause and renderEvent then
		...
		local varient = specialEvents[renderEvent].varient
		lastVarient = varient
		local source = specialEvents[renderEvent].event
		local choicesDepth = {}
		local i = 1
		while #choicesDepth < #choicesChosenList do
			...
			if chosenChoice.finish then
				...
				Hyperspace.CustomEventsParser.GetInstance():LoadEvent(worldManager, specialEvents[renderEvent].finish, false,-1)
				renderEvent = nil
			elseif chosenChoice.finish2 then
				varient = 0
				local worldManager = Hyperspace.App.world
				Hyperspace.CustomEventsParser.GetInstance():LoadEvent(worldManager, specialEvents[renderEvent].finish2, false,-1)
				renderEvent = nil
			else
